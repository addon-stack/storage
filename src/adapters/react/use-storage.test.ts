...
test("preserves null and distinguishes a stored value equal to the default", async () => {
    ...
    const {result} = renderHook(() => useStorage({storage, keys: ["theme", "language", "count"], defaultValue: {theme: "light", language: "ru"}}));
    await ready(result);
...
test("keeps keys mode as an object even for one key and supports an empty selection", async () => {
    ...
    expect(empty.result.current).toMatchObject({value: {}, exists: {}, status: "ready"});
});
...
test("shares one subscription and in-flight read across consumers, with independent defaults", async () => {
    ...
    act(() => global.simulateStorageChange({storage, key: "theme", oldValue: undefined, newValue: "blue"}));
...
test("does not re-read or resubscribe for inline options and reordered equivalent keys", async () => {
    ...
    const subscribe = jest.spyOn(storage, "subscribe");
    const {result, rerender} = renderHook(({reverse}) => useStorage({storage, keys: reverse ? ["language", "theme"] : ["theme", "language"], defaultValue: {theme: "light"}}), {initialProps: {reverse: false}});
    await ready(result);
...
test("switches providers and isolates identical keys by provider identity", async () => {
    ...
    await b.set("theme", "b");
    const {result, rerender} = renderHook(({storage}) => useStorage({storage, key: "theme"}), {initialProps: {storage: a}});
    rerender({storage: b});
    await ready(result);
...
test("read-only managed data can be observed and native mutation errors are preserved", async () => {
    ...
    await chrome.storage.managed.set({theme: "policy"});
    const {result} = renderHook(() => useStorage({storage, key: "theme"}));
    await ready(result);
