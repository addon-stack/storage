import {storageLocal, StorageStatus, type StorageUpdateOptions} from "~";
import {useStorage} from "~/adapters/react";
...
function useSingleOptions(options: import("~/adapters/react").UseStorageSingleOptions<State, "theme">) {
...
function useBatchOptions(options: import("~/adapters/react").UseStorageBatchOptions<State, "theme" | "count">) {
    const result = useStorage(options);
...
function useDynamicOptions(options: import("~/adapters/react").UseStorageOptions<State, "theme">) {
    const result = useStorage(options);

    const proof: Expect<Equal<typeof result,
        import("~/adapters/react").UseStorageReturnValue<State["theme"]> |
        import("~/adapters/react").UseStorageBatchReturnValue<State, "theme">
    >> = true;
...
function verifyPreparedOptions() {
    const single = {key: "theme", storage, defaultValue: "light" as const} satisfies import("~/adapters/react").UseStorageSingleOptions<State>;
    const batch = {keys: ["theme", "count"] as const, storage} satisfies import("~/adapters/react").UseStorageBatchOptions<State>;
...
function useGenericSingle<S extends Record<string, unknown>, K extends keyof S & string>(
    options: import("~/adapters/react").UseStorageSingleOptions<S, K>
) {
    const result = useStorage(options);
