...
test("rejects unknown options instead of treating an object as batch set", () => {
    ...
    expect(browser.storage.local.set.calls).toHaveLength(0);
});
...
test("rejects unknown secure options before encryption or native I/O", () => {
    const encrypt = jest.spyOn(crypto.subtle, "encrypt");
...
test("storageManaged reads seeded policy and preserves native read-only errors", async () => {
    await withBrowser({storage: {managed: {theme: "policy"}}}, async fixture => {
        await expect(storageManaged<string>("theme")).resolves.toBe("policy");
        await expect(storageManaged<string>("theme", "other")).rejects.toThrow("managed storage is read-only");
        expect(fixture.storage.managed.data).toEqual({theme: "policy"});
    });
});
